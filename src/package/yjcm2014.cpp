...
void DingpinCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(effect.from, "dingpinengine");
    if (effect.from->getMark("dingpinengine") > 0) {
		JudgeStruct judge;
		judge.who = effect.to;
		judge.good = true;
		judge.pattern = ".|black";
		...
		if (judge.isGood()) {
			...
			room->broadcastSkillInvoke("dingpin", 2);
			effect.to->drawCards(effect.to->getLostHp(), "dingpin");
		} else {
			effect.from->turnOver();
		}
        room->removePlayerMark(effect.from, "dingpinengine");
    }
}
...
void SidiCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    CardMoveReason reason(CardMoveReason::S_REASON_REMOVE_FROM_PILE, QString(), source->objectName(), "sidi", QString());
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        if (triggerEvent == EventPhaseChanging) {
            ...
            foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
                ...
                if (TriggerSkill::triggerable(p) && p->getPile("lock").length() > 0 && room->askForUseCard(p, "@@sidi", "sidi_remove:remove", -1, Card::MethodNone)) {
    ...
    virtual bool onPhaseChange(ServerPlayer *target) const{
        ...
        foreach (ServerPlayer *hs, room->getOtherPlayers(target)) {
            ...
            if (room->askForSkillInvoke(hs, objectName())) {
                room->broadcastSkillInvoke(objectName());
				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, hs->objectName(), target->objectName());
                ...
                if (hs->isAlive() && target->isAlive() && hs->canSlash(target, false)) {
					room->addPlayerMark(hs, objectName()+"engine");
					if (hs->getMark(objectName()+"engine") > 0) {
						...
						room->removePlayerMark(hs, objectName()+"engine");
					}
                }
            }
        }
...
class BenxiC: public TriggerSkill {
public:
    BenxiC(): TriggerSkill("#benxi") {
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        PhaseChangeStruct change = data.value<PhaseChangeStruct>();
        if (change.to == Player::NotActive && player->getMark("@benxi")) {
            foreach (ServerPlayer *p, room->getAlivePlayers()) {
				room->setPlayerMark(p, "@benxi", 0);
			}
		}
        return false;
    }
...
void ShenxingCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const{
    ...
    if (source->isAlive()) {
		room->addPlayerMark(source, "shenxingengine");
		if (source->getMark("shenxingengine") > 0) {
            room->drawCards(source, 1, "shenxing");
			room->removePlayerMark(source, "shenxingengine");
		}
    }
}
...
void BingyiCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
	room->addPlayerMark(source, "bingyiengine");
    if (source->getMark("bingyiengine") > 0) {
        room->showAllCards(source);
        foreach(ServerPlayer *p, targets)
            room->drawCards(p, 1, "bingyi");
        room->removePlayerMark(source, "bingyiengine");
    }
}
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        foreach (ServerPlayer *caifuren, room->getAllPlayers()) {
            ...
            if (choice == "cancel") {
                ...
                room->addPlayerMark(caifuren, objectName()+"engine");
                if (caifuren->getMark(objectName()+"engine") > 0) {
					if (choice == "draw") {
						room->broadcastSkillInvoke(objectName(), 2);
						caifuren->drawCards(1, objectName());
					} else {
						room->broadcastSkillInvoke(objectName(), 1);
						room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, caifuren->objectName(), player->objectName());
						int index = choice.toInt();
						const Card *card = player->getEquip(index);
						room->moveCardTo(card, caifuren, Player::PlaceEquip);
					}
                    room->removePlayerMark(caifuren, objectName()+"engine");
                }
            }
        }
        return false;
...
void XianzhouCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(effect.from, "xianzhouiengine");
    if (effect.from->getMark("xianzhouiengine") > 0) {
		bool rec = true;
		int count = 0;
        ...
        room->removePlayerMark(effect.from, "xianzhouiengine");
    }
}
...
public:
    ...
    virtual const Card *viewAs() const{
        ...
        if (pattern == "@xianzhou" || pattern == "@xianzhou!") {
