...
public:
    ...
    virtual int getDrawNum(ServerPlayer *player, int n) const{
        ...
        if (can_invoke) {
            ServerPlayer *target = room->askForPlayerChosen(player, targets, objectName(), "koftuxi-invoke", true, true);
            if (target) {
				room->addPlayerMark(player, objectName()+"engine");
				if (player->getMark(objectName()+"engine") > 0) {
					target->setFlags("KOFTuxiTarget");
					player->setFlags("koftuxi");
					n = n - 1;
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const{
        if (!player->hasFlag("koftuxi")) return false;
        player->setFlags("-koftuxi");
        ...
        int card_id = room->askForCardChosen(player, target, "h", "koftuxi");
...
void XiechanCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    room->addPlayerMark(source, "xiechanengine");
    if (source->getMark("xiechanengine") > 0) {
    room->removePlayerMark(source, "@twine");
    //room->doLightbox("$XiechanAnimate");

    bool success = source->pindian(targets.first(), "xiechan", NULL);
    ...
    if (!from->isLocked(duel) && !from->isProhibited(to, duel))
        ...
        room->removePlayerMark(source, "xiechanengine");
    }
}
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        if (room->askForSkillInvoke(xiahouyuan, objectName(), data)) {
			room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, xiahouyuan->objectName(), player->objectName());
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        if (effect.card->isKindOf("SavageAssault")) {
            ...
            room->addPlayerMark(player, "manyiengine");
            if (player->getMark("manyiengine") > 0) {
                room->removePlayerMark(player, "manyiengine");
                return true;
            }
        }
        return false;
        ...
        if (move.from == sunshangxiang && move.from_places.contains(Player::PlaceEquip)) {
            for (int i = 0; i < move.card_ids.size(); ++i) {
                ...
                if (move.from_places[i] == Player::PlaceEquip) {
                    ...
                    log.arg = objectName();
                    room->sendLog(log);
					room->addPlayerMark(sunshangxiang, objectName()+"engine");
					if (sunshangxiang->getMark(objectName()+"engine") > 0) {
						if (choice == "draw") {
						...
						room->removePlayerMark(sunshangxiang, objectName()+"engine");
					}
                }
            }
        }
        ...
        if (triggerEvent == CardUsed && player->getPhase() == Player::Play) {
            ...
            foreach (ServerPlayer *p, room->getOtherPlayers(use.from)) {
                 if (use.to.contains(p) && !first.contains(p) && p->canDiscard(use.from, "he") && p->hasFlag("RenwangEffect") && TriggerSkill::triggerable(p) && room->askForSkillInvoke(p, objectName(), data)) {
                    room->broadcastSkillInvoke(objectName()); // Renwang has no audio effect at this time.
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        if (use.card->isKindOf("Slash") && use.to.contains(player) && room->askForSkillInvoke(player, objectName(), data)) {
			room->broadcastSkillInvoke(objectName()); // wanrong has no audio effect at this time.
			room->addPlayerMark(player, objectName()+"engine");
			if (player->getMark(objectName()+"engine") > 0) {
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        if (TriggerSkill::triggerable(data.value<DeathStruct>().damage->from) && data.value<DeathStruct>().damage->from && room->askForSkillInvoke(data.value<DeathStruct>().damage->from, objectName(), data)) {
            room->broadcastSkillInvoke(objectName());
			room->addPlayerMark(data.value<DeathStruct>().damage->from, objectName()+"engine");
			if (data.value<DeathStruct>().damage->from->getMark(objectName()+"engine") > 0) {
				data.value<DeathStruct>().damage->from->drawCards(3, objectName());
				room->removePlayerMark(data.value<DeathStruct>().damage->from, objectName()+"engine");
			}
		}
        return false;
    }
...
Special1v1Package::Special1v1Package()
    : Package("Special1v1")
{
    General *kof_nos_zhangliao = new General(this, "kof_nos_zhangliao", "wei", 4, true, Config.value("EnableHidden", true).toBool());
    ...
    General *kof_nos_xuchu = new General(this, "kof_nos_xuchu", "wei", 4, true, Config.value("EnableHidden", true).toBool());
    kof_nos_xuchu->addSkill("nosluoyi");
    kof_nos_xuchu->addSkill(new Xiechan);

    General *kof_zhenji = new General(this, "kof_zhenji", "wei", 3, false, Config.value("EnableHidden", true).toBool());
    kof_zhenji->addSkill(new KOFQingguo);
    kof_zhenji->addSkill("luoshen");

    General *kof_xiahouyuan = new General(this, "kof_xiahouyuan", "wei", 4, true, Config.value("EnableHidden", true).toBool());
    kof_xiahouyuan->addSkill("shensu");
    kof_xiahouyuan->addSkill(new Suzi);

    General *kof_nos_liubei = new General(this, "kof_nos_liubei$", "shu", 4, true, Config.value("EnableHidden", true).toBool());
    kof_nos_liubei->addSkill(new Renwang);
    kof_nos_liubei->addSkill("jijiang");

    General *kof_nos_guanyu = new General(this, "kof_nos_guanyu", "shu", 4, true, Config.value("EnableHidden", true).toBool());
    kof_nos_guanyu->addSkill("wusheng");
    kof_nos_guanyu->addSkill(new Huwei);

    General *kof_nos_huangyueying = new General(this, "kof_nos_huangyueying", "shu", 3, false, Config.value("EnableHidden", true).toBool());
    ...
    General *kof_huangzhong = new General(this, "kof_huangzhong", "shu", 4, true, Config.value("EnableHidden", true).toBool());
    kof_huangzhong->addSkill(new KOFLiegong);

    General *kof_weiyan = new General(this, "kof_weiyan", "shu", 4, true, Config.value("EnableHidden", true).toBool());
    kof_weiyan->addSkill(new KOFKuanggu);

    General *kof_jiangwei = new General(this, "kof_jiangwei", "shu", 4, true, Config.value("EnableHidden", true).toBool());
    kof_jiangwei->addSkill("tiaoxin");

    General *kof_menghuo = new General(this, "kof_menghuo", "shu", 4, true, Config.value("EnableHidden", true).toBool());
    ...
    General *kof_zhurong = new General(this, "kof_zhurong", "shu", 4, false, Config.value("EnableHidden", true).toBool());
    kof_zhurong->addSkill("manyi");
    kof_zhurong->addSkill("lieren");

    General *kof_nos_lvmeng = new General(this, "kof_nos_lvmeng", "wu", 4, true, Config.value("EnableHidden", true).toBool());
    ...
    General *kof_nos_daqiao = new General(this, "kof_nos_daqiao", "wu", 3, false, Config.value("EnableHidden", true).toBool());
    kof_nos_daqiao->addSkill("nosguose");
    kof_nos_daqiao->addSkill(new Wanrong);

    General *kof_sunshangxiang = new General(this, "kof_sunshangxiang", "wu", 3, false, Config.value("EnableHidden", true).toBool());
    kof_sunshangxiang->addSkill(new Yinli);
    kof_sunshangxiang->addSkill(new KOFXiaoji);

    General *kof_nos_huatuo = new General(this, "kof_nos_huatuo", "qun", 3, true, Config.value("EnableHidden", true).toBool());
    kof_nos_huatuo->addSkill("jijiu");
    kof_nos_huatuo->addSkill(new Puji);

    General *kof_nos_diaochan = new General(this, "kof_nos_diaochan", "qun", 3, false, Config.value("EnableHidden", true).toBool());
