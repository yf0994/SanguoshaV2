    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == CardResponded) {
            ...
            if (resp.m_card->getSkillName() == "longdan"
                && resp.m_who != NULL && !resp.m_who->isKongcheng() && resp.m_who != player) {
...
void LihunCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->addPlayerMark(effect.from, "lihunengine");
    if (effect.from->getMark("lihunengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "lihunengine");
    }
}
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        if (player->getPhase() == Player::Finish) {
			...
			if (player->getMark(objectName()+"engine") > 0) {
				...
				player->drawCards(n + 2, objectName());
				player->turnOver();

				if (player->getMark("@kuiwei") == 0)
					room->addPlayerMark(player, "@kuiwei");
				room->removePlayerMark(player, objectName()+"engine");
			}
...
public:
    ...
    {
        if (player->hasFlag("ManjuanInvoke")) {
        ...
        QList<int> ids;
        CardMoveReason reason(CardMoveReason::S_REASON_PUT, player->objectName(), "manjuan", QString());
		...
		if (player->getMark(objectName()+"engine") > 0) {
			...
			if (player->getPhase() == Player::NotActive || !room->askForSkillInvoke(player, objectName(), data))
				return false;

			room->removePlayerMark(player, objectName()+"engine");
			foreach (int _card_id, ids) {
				doManjuan(player, _card_id);
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == EventPhaseStart && player->getMark("zuixiangHasTrigger") == 0) {
            if (player->getPhase() == Player::Start) {
                if (TriggerSkill::triggerable(player) && player->getMark("@sleep") > 0) {
                    if (!room->askForSkillInvoke(player, objectName()))
                        return false;
					room->addPlayerMark(player, objectName()+"engine");
					if (player->getMark(objectName()+"engine") > 0) {
                ...
                } else if (!player->getPile("dream").isEmpty())
                    doZuixiang(player);
            }
        } else if (triggerEvent == CardEffected && TriggerSkill::triggerable(player)) {
            if (zuixiang.isEmpty())
                return false;
        ...
        } else if (triggerEvent == SlashEffected && TriggerSkill::triggerable(player)) {
            if (zuixiang.isEmpty())
                return false;
...
void DaheCard::use(Room *room, ServerPlayer *zhangfei, QList<ServerPlayer *> &targets) const
{
    ...
    room->addPlayerMark(zhangfei, "daheengine");
    if (zhangfei->getMark("daheengine") > 0) {
		zhangfei->pindian(targets.first(), "dahe", NULL);
        room->removePlayerMark(zhangfei, "daheengine");
    }
}
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *, QVariant &data) const
    {
        ...
        if (pindian->isSuccess()) {
            ...
            if (!to_givelist.isEmpty()) {
				...
				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, pindian->from->objectName(), to_give->objectName());
...
void TanhuCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    room->addPlayerMark(source, "tanhuengine");
    if (source->getMark("tanhuengine") > 0) {
		bool success = source->pindian(targets.first(), "tanhu", NULL);
		if (success) {
			room->broadcastSkillInvoke("tanhu", 2);
			source->tag["TanhuInvoke"] = QVariant::fromValue(targets.first());
			targets.first()->setFlags("TanhuTarget");
			room->setFixedDistance(source, targets.first(), 1);
		} else {
			room->broadcastSkillInvoke("tanhu", 3);
		}
        room->removePlayerMark(source, "tanhuengine");
    }
}
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        if (triggerEvent == GameStart) {
			room->addPlayerMark(player, "mouduanengine");
			if (player->getMark("mouduanengine") > 0) {
				player->gainMark("@wu");
				room->handleAcquireDetachSkills(player, "jiang|qianxun");
				room->removePlayerMark(player, "mouduanengine");
			}
        } else if (data.toString() == "mouduan") {
            if (player->getMark("@wu") > 0)
                room->handleAcquireDetachSkills(player, "jiang|qianxun");
            else if (player->getMark("@wen") > 0)
                room->handleAcquireDetachSkills(player, "yingzi|keji");
        }
        return false;
    }
};
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == CardsMoveOneTime) {
            ...
            if (lvmeng->getHandcardNum() > 2) {
                room->broadcastSkillInvoke(objectName());
				room->addPlayerMark(lvmeng, objectName()+"engine");
				if (lvmeng->getMark(objectName()+"engine") > 0) {
					lvmeng->loseMark("@wen");
					lvmeng->gainMark("@wu");
					room->handleAcquireDetachSkills(lvmeng, "-nosyingzi|-keji|jiang|nosqianxun", true);
					room->removePlayerMark(lvmeng, objectName()+"engine");
				}
            }
        }
        return false;
...
public:
    ...
    virtual int getDrawNum(ServerPlayer *player, int n) const{
        ...
        if (victims.isEmpty())
            return n;
        ServerPlayer *victim = room->askForPlayerChosen(player, victims, "zhaolie", "zhaolie-invoke", true, true);
        if (victim) {
			room->addPlayerMark(player, objectName()+"engine");
			if (player->getMark(objectName()+"engine") > 0) {
				victim->setFlags("ZhaolieTarget");
				player->setFlags("zhaolie");
				room->removePlayerMark(player, objectName()+"engine");
			}
            return n - 1;
        }
...
void ShichouCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(player, "shichouengine");
    if (player->getMark("shichouengine") > 0) {
        ...
        room->removePlayerMark(player, "shichouengine");
    }
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        if (triggerEvent == DamageCaused) {
            ...
            CardUseStruct use = data.value<CardUseStruct>();
            if (!use.to.contains(player) || !player->canDiscard(player, "h"))
                return false;
            if (use.card->isKindOf("Slash")) {
                if (room->askForCard(player, ".", "@anxian-discard", data, objectName())) {
                    room->broadcastSkillInvoke(objectName(), 2);
					room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, player->objectName(), use.from->objectName());
					room->addPlayerMark(player, objectName()+"engine");
					if (player->getMark(objectName()+"engine") > 0) {
						player->setFlags("-AnxianTarget");
						player->setFlags("AnxianTarget");
						use.from->drawCards(1, objectName());
						if (player->isAlive() && player->hasFlag("AnxianTarget")) {
							player->setFlags("-AnxianTarget");
...
void YinlingCard::onEffect(const CardEffectStruct &effect) const{
    ...
    if (!effect.from->canDiscard(effect.to, "he") || effect.from->getPile("brocade").length() >= 4)
        return;
    room->addPlayerMark(effect.from, "yinlingengine");
    if (effect.from->getMark("yinlingengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "yinlingengine");
    }
}
...
void JunweiCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->addPlayerMark(effect.from, "junwenengine");
    if (effect.from->getMark("junwenengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "junwenengine");
    }
}
		...
		if (player->getPhase() == Player::Finish) {
            foreach (ServerPlayer *xiahou, room->findPlayersBySkillName(objectName())) {
				if (xiahou->getMark("@fenyong") > 0) {
					...
					room->addPlayerMark(xiahou, objectName()+"engine");
					if (xiahou->getMark(objectName()+"engine") > 0) {
						...
						if (choice == "slash") {
							...
							room->broadcastSkillInvoke(objectName(), 1);
							room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, xiahou->objectName(), player->objectName());
						...
						room->removePlayerMark(xiahou, objectName()+"engine");
					}
				}
			}
		}
...
BGMPackage::BGMPackage(): Package("BGM") {
    General *bgm_zhaoyun = new General(this, "bgm_zhaoyun", "qun", 3, true, Config.value("EnableHidden", true).toBool()); // *SP 001
    bgm_zhaoyun->addSkill("longdan");
    bgm_zhaoyun->addSkill(new Chongzhen);

    General *bgm_diaochan = new General(this, "bgm_diaochan", "qun", 3, false, Config.value("EnableHidden", true).toBool()); // *SP 002
    bgm_diaochan->addSkill(new Lihun);
    bgm_diaochan->addSkill("biyue");

    General *bgm_caoren = new General(this, "bgm_caoren", "wei", 4, true, Config.value("EnableHidden", true).toBool()); // *SP 003
    bgm_caoren->addSkill(new Kuiwei);
    bgm_caoren->addSkill(new Yanzheng);

    General *bgm_pangtong = new General(this, "bgm_pangtong", "qun", 3, true, Config.value("EnableHidden", true).toBool()); // *SP 004
    ...
    General *bgm_zhangfei = new General(this, "bgm_zhangfei", "shu", 4, true, Config.value("EnableHidden", true).toBool()); // *SP 005
    ...
    General *bgm_lvmeng = new General(this, "bgm_lvmeng", "wu", 3, true, Config.value("EnableHidden", true).toBool()); // *SP 006
    ...
    General *bgm_liubei = new General(this, "bgm_liubei$", "shu", 4, true, Config.value("EnableHidden", true).toBool()); // *SP 007
    ...
    General *bgm_daqiao = new General(this, "bgm_daqiao", "wu", 3, false, Config.value("EnableHidden", true).toBool()); // *SP 008
    bgm_daqiao->addSkill(new Yanxiao);
    bgm_daqiao->addSkill(new Anxian);

    General *bgm_ganning = new General(this, "bgm_ganning", "qun", 4, true, Config.value("EnableHidden", true).toBool()); // *SP 009
    ...
    General *bgm_xiahoudun = new General(this, "bgm_xiahoudun", "wei", 4, true, Config.value("EnableHidden", true).toBool()); // *SP 010
...
void ZhaoxinCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    ...
    room->addPlayerMark(source, "zhaoxinengine");
    if (source->getMark("zhaoxinengine") > 0) {

		Slash *slash = new Slash(Card::NoSuit, 0);
		slash->setSkillName("_zhaoxin");
		room->useCard(CardUseStruct(slash, source, targets));
        room->removePlayerMark(source, "zhaoxinengine");
    }
}
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *simazhao, QVariant &data) const{
        if (TriggerSkill::triggerable(simazhao) && triggerEvent == Damaged) {
            ...
            for (int i = 0; i < damage.damage; ++i) {
                if (!simazhao->isAlive() || !room->askForSkillInvoke(simazhao, objectName(), data))
                    return false;
                room->broadcastSkillInvoke(objectName());

				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, simazhao->objectName(), damage.from->objectName());
				room->addPlayerMark(simazhao, objectName()+"engine");
				if (simazhao->getMark(objectName()+"engine") > 0) {
					JudgeStruct judge;
					judge.good = true;
					...
					room->removePlayerMark(simazhao, objectName()+"engine");
				}
            }
...
void FuluanCard::onEffect(const CardEffectStruct &effect) const{
    Room *room = effect.to->getRoom();
    room->addPlayerMark(effect.from, "fuluanengine");
    if (effect.from->getMark("fuluanengine") > 0) {
		effect.to->turnOver();
		room->setPlayerCardLimitation(effect.from, "use", "Slash", true);
        room->removePlayerMark(effect.from, "fuluanengine");
    }
}
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const{
        if (player->getPhase() == Player::Finish) {
            ...
            if (x > 0 && room->askForSkillInvoke(player, objectName())) {
                room->broadcastSkillInvoke(objectName());
				room->addPlayerMark(player, objectName()+"engine");
				if (player->getMark(objectName()+"engine") > 0) {
...
void HuangenCard::onEffect(const CardEffectStruct &effect) const{
    Room *room = effect.from->getRoom();
    room->addPlayerMark(effect.from, "huangenengine");
    if (effect.from->getMark("huangenengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "huangenengine");
    }
}
        ...
        if (move.to_place == Player::DiscardPile
            ...
            if (room->askForSkillInvoke(player, objectName())) {
				...
				if (player->getMark(objectName()+"engine") > 0) {
					...
					data = QVariant::fromValue(move);
					if (!to_add.isEmpty())
						player->addToPile("edict", to_add, true, QList<ServerPlayer *>(), move.reason);
					room->removePlayerMark(player, objectName()+"engine");
				}
            }
        }
        ...
        case CardAsked: {
                ...
                if (pattern == "slash" && !liuxie->hasFlag("Global_JijiangFailed")) {
                    ...
                    if (room->askForSkillInvoke(liuxie, "hantong_acquire", data_for_ai)) {
                        RemoveEdict(liuxie);
						room->addPlayerMark(liuxie, "hantongengine");
						if (liuxie->getMark("hantongengine") > 0) {
							room->acquireSkill(liuxie, "jijiang");
							room->removePlayerMark(liuxie, "hantongengine");
						}
                    }
...
void DIYYicongCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const{
    room->addPlayerMark(source, "diyyicongengine");
    if (source->getMark("diyyicongengine") > 0) {
		source->addToPile("retinue", this);
        room->removePlayerMark(source, "diyyicongengine");
    }
}
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        if (triggerEvent == EventPhaseStart) {
            if (player->getPhase() == Player::Start && player->getPile("retinue").length() > 0) {
                room->sendCompulsoryTriggerLog(player, objectName());

                int n = player->getPile("retinue").length();
