...
public:
    ...
    const Card *viewAs(const QList<const Card *> &cards) const
    {
        ...
        switch (hp) {
        case 1:
			if (Config.value("wu_down", true).toBool()) {
				if (cards.length() > 0) {
					NosRendeCard *gs = new NosRendeCard;
    ...
    bool isEnabledAtPlay(const Player *player) const
    {
        ...
        switch (hp) {
        case 1:
            return Config.value("wu_down", true).toBool() || !player->hasUsed("RendeCard");
            break;
        case 2:
            return false; // Trigger Skill
            break;
        case 3:
            ...
            break;
        case 4:
            return Config.value("wu_down", true).toBool() || !player->hasUsed("GuoseCard");
            break;
        case 5:
...
public:
    ...
    {
        CardUseStruct use = data.value<CardUseStruct>();
        const TriggerSkill *jz = Sanguosha->getTriggerSkill(Config.value("wu_down", true).toBool() ? "nosjizhi" : "jizhi");
