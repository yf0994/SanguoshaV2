...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *shenguanyu, QVariant &data) const
    {
        ...
        room->addPlayerMark(shenguanyu, objectName()+"engine");
        if (shenguanyu->getMark(objectName()+"engine") > 0) {
            JudgeStruct judge;
            ...
            if (judge.isBad()) {
                ...
                room->killPlayer(foe);
            } else if (!Config.value("music", true).toBool())
            ...
            room->removePlayerMark(shenguanyu, objectName()+"engine");
        }
        return false;
    }
};
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const{
        ...
        if (player->getMark(objectName()+"engine") > 0) {
            ...
            while (!card_ids.isEmpty()) {
                int card_id = room->askForAG(player, card_ids, false, "shelie");
...
void GongxinCard::onEffect(const CardEffectStruct &effect) const{
    ...
    if (!effect.to->isKongcheng()) {
        room->addPlayerMark(effect.from, "gongxinengine");
        if (effect.from->getMark("gongxinengine") > 0) {
            QList<int> ids;
            ...
            int card_id = room->doGongxin(effect.from, effect.to, ids, "gongxin");
            if (card_id == -1) return;
            ...
            room->removePlayerMark(effect.from, "gongxinengine");
        }
    }
}
...
public:
    ...
    virtual int getEffectIndex(const ServerPlayer *player, const Card *) const{
        int index = qrand() % 2 + 1;
        if (!player->hasInnateSkill(objectName()) && (Config.value("heg_skill", true).toBool() || player->getMark("qinxue") > 0))
            index += 2;
        return index;
    }
};
...
void GreatYeyanCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    ...
    if (criticaltarget > 0) {
		 room->broadcastSkillInvoke("yeyan", !Config.value("music", true).toBool() && totalvictim == 1 ? 3 : 2);
...
public:
    ...
    void perform(ServerPlayer *shenzhouyu) const{
        ...
        room->addPlayerMark(shenzhouyu, objectName()+"engine");
        if (shenzhouyu->getMark(objectName()+"engine") > 0) {
            foreach(ServerPlayer *p, all_players)
				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, shenzhouyu->objectName(), p->objectName());
            if (result == "up") {
                ...
                int index = 1;
                if (room->findPlayer("caocao+shencaocao+yt_shencaocao") && !Config.value("music", true).toBool())
                    index = 3;
            ...
            room->removePlayerMark(shenzhouyu, objectName()+"engine");
        }
    }
            ...
            for (int i = 0; i < damage.damage; i++) {
                ...
                if (shencc->askForSkillInvoke(objectName(), data)) {
                   ...
                   // else
                    ...
                    room->addPlayerMark(shencc, objectName()+"engine");
                    if (shencc->getMark(objectName()+"engine") > 0) {
                        foreach (ServerPlayer *player, players) {
                        ...
                        foreach (ServerPlayer *player, players) {
                            if (player->isAlive() && !player->isAllNude()) {
								...
								int id;
								if (Config.value("caocao_down", true).toBool()) {
									QStringList choices;
									choices << "h" << "e" << "j";
									QStringList copy;
									copy << "h" << "e" << "j";
									for (int i = 0; i < 3; i++) {
										if (player->getCards(choices[i]).isEmpty())
											copy.removeOne(choices[i]);
										
									}
									QString choice = room->askForChoice(player, objectName(), copy.join("+"), QVariant::fromValue(player));
									id = player->getCards(choice).at(qrand() % (player->getCards(choice).length() - 1))->getId();
                                } else {
									id = room->askForCardChosen(shencc, player, "hej", objectName());
								}
								room->obtainCard(shencc, Sanguosha->getCard(id), reason, room->getCardPlace(id) != Player::PlaceHand);
                                if (shencc->isDead()) {
                                    shencc->setFlags("-GuixinUsing");
                                    shencc->setMark("GuixinTimes", 0);
                                    return;
                        ...
                        shencc->turnOver();
                        room->removePlayerMark(shencc, objectName()+"engine");
                    }
...
void ShenfenCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const{
    source->setFlags("ShenfenUsing");
    room->broadcastSkillInvoke("shenfen");
    //QString lightbox = "$ShenfenAnimate";
    //if (source->getGeneralName() != "shenlvbu" && (source->getGeneralName() == "sp_shenlvbu" || source->getGeneral2Name() == "sp_shenlvbu"))
    ...
    try {
        ...
        if (source->getMark(objectName()+"engine") > 0) {
            foreach (ServerPlayer *player, players) {
                room->damage(DamageStruct("shenfen", source, player));
...
void QixingCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    ...
    room->addPlayerMark(card_use.from, objectName()+"engine");
    if (card_use.from->getMark(objectName()+"engine") > 0) {
        ...
        log.arg2 = "qixing";
        room->sendLog(log);
        room->removePlayerMark(card_use.from, objectName()+"engine");
    }
}
...
public:
    ...
    {
        room->askForUseCard(player, "@@qixing", "@qixing-exchange", -1, Card::MethodNone);
        return false;
    }
};
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        if (triggerEvent == DrawInitialCards) {
            ...
            if (player->getMark(objectName()+"engine") > 0) {
				const Card *exchange_card = room->askForExchange(player, "qixing", 7, 7);
...
class KuangfengAsk: public PhaseChangeSkill {
public:
    KuangfengAsk(): PhaseChangeSkill("#kuangfeng-ask") {
    }
...
class DawuAsk: public PhaseChangeSkill {
public:
    DawuAsk(): PhaseChangeSkill("#dawu-ask") {
    }
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *shensimayi) const{
        ...
        room->addPlayerMark(shensimayi, objectName()+"engine");
        if (shensimayi->getMark(objectName()+"engine") > 0) {
            ...
            room->removePlayerMark(shensimayi, objectName()+"engine");
        }
        return false;
    }
};
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        try {
            if (triggerEvent == CardUsed) {
                const TriggerSkill *jizhi = Sanguosha->getTriggerSkill(Config.value("simayi_duang", true).toBool() ? "nosjizhi" : "jizhi");
                CardUseStruct use = data.value<CardUseStruct>();
                if (jizhi && use.card && ((Config.value("simayi_duang", true).toBool() && use.card->isNDTrick()) || (!Config.value("simayi_duang", true).toBool() && use.card->isKindOf("TrickCard"))) && player->askForSkillInvoke("jilve_jizhi", data)) {
                    room->notifySkillInvoked(player, objectName());
                    player->loseMark("@bear");
                    jizhi->trigger(triggerEvent, room, player, data);
                }
            } else if (triggerEvent == AskForRetrial) {
                const TriggerSkill *guicai = Sanguosha->getTriggerSkill(Config.value("simayi_down", true).toBool() ? "nosguicai" : "guicai");
...
public:
    ...
    virtual int getDrawNum(ServerPlayer *player, int n) const{
        ...
        if (player->isWounded()) {
            ...
            log.arg2 = "juejing";
            room->sendLog(log);
			room->addPlayerMark(player, "juejingengine");
			if (player->getMark("juejingengine") > 0) {
				n = n + player->getLostHp();
				room->removePlayerMark(player, "juejingengine");
			}
        }
        return n;
    }
...
int Longhun::getEffectIndex(const ServerPlayer *player, const Card *card) const{
	if (Config.value("music", true).toBool()) return qrand() % 4 + 1;
...
public:
    ...
    virtual bool isProhibited(const Player *from, const Player *to, const Card *card, const QList<const Player *> &) const{
		return card->isNDTrick() && from != to && to->hasArmorEffect("god_pao");
    }
};
    ...
    : Package("god")
{
    ...
    shenzhugeliang->addSkill(new DawuAsk);
    shenzhugeliang->addSkill(new KuangfengAsk);
    ...
    related_skills.insertMulti("kuangfeng", "#kuangfeng-ask");
    related_skills.insertMulti("dawu", "#dawu-ask");
    ...
    skills << new Jilve << new JilveClear << new GodPaoSkill;
}
