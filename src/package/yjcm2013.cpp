            ...
            foreach (ServerPlayer *p, room->getOtherPlayers(damage.to)) {
                if (TriggerSkill::triggerable(p) && p->canDiscard(p, "he")) {
					if (room->askForCard(p, ".Equip", "@renxin-card:" + damage.to->objectName(), data, objectName())) {
						room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, p->objectName(), damage.to->objectName());
...
void JunxingCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    ...
    room->addPlayerMark(source, "junxingengine");
    if (source->getMark("junxingengine") > 0) {
        ...
        room->removePlayerMark(source, "junxingengine");
    }
}
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *, QVariant &data) const{
        ...
        if (jianyong && to_obtain && room->getCardPlace(to_obtain->getEffectiveId()) == Player::PlaceTable
            ...
            room->addPlayerMark(jianyong, objectName()+"engine");
            if (jianyong->getMark(objectName()+"engine") > 0) {
                jianyong->obtainCard(to_obtain);
                room->removePlayerMark(jianyong, objectName()+"engine");
            }
        }
...
void XiansiCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(effect.from, "xiansiengine");
    if (effect.from->getMark("xiansiengine") > 0) {
		int id = effect.from->getRoom()->askForCardChosen(effect.from, effect.to, "he", "xiansi");
        effect.from->addToPile("counter", id);
        room->removePlayerMark(effect.from, "xiansiengine");
    }
}
...
void ZongxuanCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const{
    room->addPlayerMark(source, "zongxuanengine");
    if (source->getMark("zongxuanengine") > 0) {
        QVariantList subcardsList;
        foreach(int id, subcards)
            subcardsList << id;
        source->tag["zongxuan"] = QVariant::fromValue(subcardsList);
        room->removePlayerMark(source, "zongxuanengine");
    }
}
...
void DanshouCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(effect.from, "danshouengine");
    if (effect.from->getMark("danshouengine") > 0) {
		switch (len) {
		case 0:
				Q_ASSERT(false);
				break;
        ...
        room->removePlayerMark(effect.from, "danshouengine");
    }
}
...
void MiejiCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(effect.from, "miejiengine");
    if (effect.from->getMark("miejiengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "miejiengine");
    }
}
...
void FenchengCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const{
    ...
    room->addPlayerMark(source, "fenchengengine");
    if (source->getMark("fenchengengine") > 0) {
        ...
        room->removePlayerMark(source, "fenchengengine");
    }
}
