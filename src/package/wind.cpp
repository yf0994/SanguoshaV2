...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        } else if (triggerEvent == DamageCaused && player->isAlive() && player->isWounded()) {
...
void HuangtianCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    ...
    if (zhangjiao->hasLordSkill("huangtian")) {
        ...
        room->addPlayerMark(source, "huangtianengine");
        if (source->getMark("huangtianengine") > 0) {
            ...
            room->removePlayerMark(source, "huangtianengine");
        }
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
		...
		if (Config.value("caoren_down", true).toBool() && turn.name != "jushou" && turn.name != "gamerule") return false;
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        if (player->getMark(objectName()+"engine") > 0) {
            ...
            foreach (int card_id, player->getPile("trauma")) {
                if (Sanguosha->getCard(card_id)->getNumber() == num) {
                    duplicate = true;
                    break;
                }
            }
            player->addToPile("trauma", id);
            if (duplicate) {
...
public:
    ...
    virtual int getFixed(const Player *target) const{
        int len = target->getPile("trauma").length();
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
        ...
        if (player->getHp() > 0 && move.from && move.from->isAlive() && move.from_places.contains(Player::PlaceHand)
            ...
            bool invoke = Config.value("zhoutai_down", true).toBool() ? room->askForDiscard(player, objectName(), 1, 1, true, true, "@fenji-discard") : room->askForSkillInvoke(player, objectName(), data);
			if (Config.value("zhoutai_down", true).toBool()){
				LogMessage log;
...
void TianxiangCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->addPlayerMark(effect.from, "tianxiangengine");
    if (effect.from->getMark("tianxiangengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "tianxiangengine");
    }
}
