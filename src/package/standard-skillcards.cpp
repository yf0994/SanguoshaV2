...
void FanjianCard::onEffect(const CardEffectStruct &effect) const{
    ...
    room->broadcastSkillInvoke("fanjian", 2);
    ...
    if (target->isAlive()) {
        if (target->isNude()) {
            ...
            if (room->askForSkillInvoke(target, "fanjian_discard", "prompt:::" + Card::Suit2String(suit))) {
				...
				room->broadcastSkillInvoke("fanjian", 1);
                room->loseHp(target);
            }
        }
    }
...
void LijianCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    room->addPlayerMark(source, "lijianengine");
    if (source->getMark("lijianengine") > 0) {
		...
		if (!from->isCardLimited(duel, Card::MethodUse) && !from->isProhibited(to, duel))
			...
			if (source->getMark("lijianengine") > 0) {
				...
				room->removePlayerMark(source, "lijianengine");
			}
		else
			delete duel;
        room->removePlayerMark(source, "lijianengine");
...
void ChuliCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    room->addPlayerMark(source, "chuliengine");
    if (source->getMark("chuliengine") > 0) {
        ...
        room->removePlayerMark(source, "chuliengine");
    }
}
...
void LiuliCard::onEffect(const CardEffectStruct &effect) const{
    Room *room = effect.from->getRoom();
    room->addPlayerMark(effect.from, "liuliengine");
    if (effect.from->getMark("liuliengine") > 0) {
		effect.to->setFlags("LiuliTarget");
        room->removePlayerMark(effect.from, "liuliengine");
    }
}
...
void FenweiCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    ...
    room->addPlayerMark(source, "fenweiengine");
    if (source->getMark("fenweiengine") > 0) {
        ...
        room->removePlayerMark(source, "fenweiengine");
    }
}
...
void GuoseCard::onUse(Room *room, const CardUseStruct &use) const{
    ...
    room->addPlayerMark(use.from, "guoseengine");
    if (use.from->getMark("guoseengine") > 0) {
        ...
        card_use = data.value<CardUseStruct>();
        thread->trigger(CardFinished, room, card_use.from, data);
        room->removePlayerMark(use.from, "guoseengine");
    }
}
...
const Card *JijiangCard::validate(CardUseStruct &cardUse) const{
    ...
    if (!liubei->hasFlag("qinwangjijiang")) {
		if (liubei->hasSkill("weidi")) {
			...
			int r = 1 + qrand() % 2;
		   if (!liubei->hasInnateSkill("jijiang") && (Config.value("heg_skill", true).toBool() || liubei->getMark("ruoyu") > 0))
				r += 2;
    ...
    foreach (ServerPlayer *liege, lieges) {
        ...
        if (slash && !slash->hasFlag("response_failed")) {
			room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, liege->objectName(), liubei->objectName());
            foreach (ServerPlayer *target, targets)
                target->setFlags("-JijiangTarget");
...
void YijiCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    foreach (ServerPlayer *target, targets) {
        ...
        target->addToPile("ruse", dummy, false);
        delete dummy;
    }
}
...
void JianyanCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const{
    room->addPlayerMark(source, "jianyanengine");
    if (source->getMark("jianyanengine") > 0) {
        ...
        room->removePlayerMark(source, "jianyanengine");
    }
}
