    ...
    virtual void onDamaged(ServerPlayer *target, const DamageStruct &damage) const{
        ...
        for (int i = 1; i <= damage.damage; ++i) {
            ...
            if (room->askForSkillInvoke(target, objectName(), QVariant::fromValue(current))) {
				...
				room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, target->objectName(), current->objectName());
...
{
    room->addPlayerMark(source, "guixiuengine");
    if (source->getMark("guixiuengine") > 0) {
		room->removePlayerMark(source, "guixiu");
		source->drawCards(2, "guixiu");
        room->removePlayerMark(source, "guixiuengine");
    }
}
...
void CunsiCard::onEffect(const CardEffectStruct &effect) const
{
	...
	//room->doLightbox("$CunsiAnimate", 5000);


    room->addPlayerMark(effect.from, "cunsiengine");
    if (effect.from->getMark("cunsiengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "cunsiengine");
    }
}
        ...
        if (move.from_places.contains(Player::PlaceTable) && move.to_place == Player::DiscardPile
            ...
            if (yongjue_user) {
                if (room->askForSkillInvoke(player, objectName(), QVariant::fromValue(yongjue_user))) {
					...
					room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, player->objectName(), yongjue_user->objectName());
					room->addPlayerMark(player, objectName()+"engine");
					if (player->getMark(objectName()+"engine") > 0) {
...
void DuanxieCard::onEffect(const CardEffectStruct &effect) const{
    Room *room = effect.from->getRoom();
    room->addPlayerMark(effect.from, "duanxieengine");
    if (effect.from->getMark("duanxieengine") > 0) {
        ...
        room->removePlayerMark(effect.from, "duanxieengine");
    }
}
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *, QVariant &data) const{
        ...
        if (TriggerSkill::triggerable(pindian->from)) {
			...
			room->addPlayerMark(pindian->from, objectName()+"engine");
			if (pindian->from->getMark(objectName()+"engine") > 0) {
				if (choice == "up") {
				...
				room->removePlayerMark(pindian->from, objectName()+"engine");
			}
        }
        if (TriggerSkill::triggerable(pindian->to)) {
            QString choice = room->askForChoice(pindian->to, objectName(), "up+down+cancel", data);
			room->addPlayerMark(pindian->to, objectName()+"engine");
			if (pindian->to->getMark(objectName()+"engine") > 0) {
				if (choice == "up") {
				...
				room->removePlayerMark(pindian->to, objectName()+"engine");
			}
        }
        return false;
    }
    ...
    : Package("h_momentum")
{
    ...
    General *heg_sunce = new General(this, "heg_sunce$", "wu", 4, true, Config.value("EnableHidden", true).toBool()); // WU 010 G
    ...
    General *heg_dongzhuo = new General(this, "heg_dongzhuo$", "qun", 4, true, Config.value("EnableHidden", true).toBool()); // QUN 006 G
