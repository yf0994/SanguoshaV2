...
#define THIS_SLASH (slash == NULL ? newslash : slash)
    ...
    if (player->getPhase() == Player::Play && (Sanguosha->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY || Sanguosha->getCurrentCardUsePattern() == "@@rende_basic")) {
        QList<int> ids;
        if (slash) {
   ...
   if (((use.card->isVirtualCard() && use.card->subcardsLength() == 0) || (getSkillName().contains("guhuo") && use.card != this)) && !player->hasFlag("slashDisableExtraTarget")) {
    ...
    if (player->getPhase() == Player::Play && player->hasFlag("Global_MoreSlashInOneTurn")) {
        if (player->hasSkill("paoxiao"))
            ...
            name = "huxiao";
        else if (player->hasFlag("TianyiSuccess"))
            name = "tianyi";
        if (!name.isEmpty()) {
            ...
            if (name == "paoxiao") {
                if (Player::isNostalGeneral(player, "zhangfei")) {
                    ...
                    if (player->hasSkill("baobian")) {
                        ...
                        index = 7;
                    } else if (player->hasSkill("nosfuhun")) {
                        index = 8;
                    }
                }
            } else if (name == "tianyi") {
			...
			if (name != "tianyi") {
				room->addPlayerMark(player, name+"engine");
				if (player->getMark(name+"engine") > 0) {
					room->removePlayerMark(player, name+"engine");
				}
			}
        }
    }
    if (use.to.size() > 1 && player->hasSkill("shenji")) {
		...
		room->addPlayerMark(player, "shenjiengine");
		if (player->getMark("shenjiengine") > 0) {
			room->removePlayerMark(player, "shenjiengine");
		}
    } else if (name != "tianyi" && use.to.size() > 1 && player->hasFlag("TianyiSuccess")) {
        room->broadcastSkillInvoke("tianyi", 2);
        room->notifySkillInvoked(player, "tianyi");
		...
		room->addPlayerMark(player, "lihuoengine");
		if (player->getMark("lihuoengine") > 0) {
			room->removePlayerMark(player, "lihuoengine");
		}
    } else if (use.to.size() > 1 && player->hasSkill("duanbing")) {
        room->broadcastSkillInvoke("duanbing");
        room->notifySkillInvoked(player, "duanbing");
		room->addPlayerMark(player, "duanbingengine");
		if (player->getMark("duanbingengine") > 0) {
			room->removePlayerMark(player, "duanbingengine");
		}
    }
...
bool Slash::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const{
    ...
    bool distance_limit = ((1 + Sanguosha->correctCardTarget(TargetModSkill::DistanceLimit, Self, this)) < 500, to_select);
...
bool Peach::isAvailable(const Player *player) const{
    bool invoke = player->hasSkill("futai") && player->getPhase() == Player::NotActive;
    foreach (const Player *p, player->getAliveSiblings()) {
        if (!invoke)
            invoke = p->hasSkill("futai") && p->getPhase() == Player::NotActive;
    }
    return !invoke && player->isWounded() && !player->isProhibited(player, this) && BasicCard::isAvailable(player);
}
...
void SavageAssault::onEffect(const CardEffectStruct &effect) const{
    Room *room = effect.to->getRoom();
    if ((!effect.from->hasSkill("fuji") || effect.to->distanceTo(effect.from) != 1) && !this->hasFlag("fuji"+effect.to->objectName()))  {
...
void Collateral::onEffect(const CardEffectStruct &effect) const
{
    ...
    if ((!effect.from->hasSkill("fuji") || effect.to->distanceTo(effect.from) != 1) && !effect.to->hasFlag("fuji" + effect.to->objectName()))  {
		if (!victim || victim->isDead()) {
			if (source->isAlive() && killer->isAlive() && killer->getWeapon())
				source->obtainCard(killer->getWeapon());
		} else {
			if (!killer->isDead() && !doCollateral(room, killer, victim, QString("collateral-slash:%1:%2").arg(victim->objectName()).arg(source->objectName()))){
...
public:
    ...
    virtual int getCorrect(const Player *from, const Player *to) const{
        ...
        if (to->getDefensiveHorse() && to->getDefensiveHorse()->objectName() != "god_horse" && to->getMark("Equips_Nullified_to_Yourself") == 0) {
			horse = qobject_cast<const Horse *>(to->getDefensiveHorse()->getRealCard());
			if (horse) {
				correct += horse->getCorrect();
			}
        }
		QString from_role = from->isLord() ? "loyalist" : from->getRole();
		QString to_role = to->isLord() ? "loyalist" : to->getRole();
        foreach (const Player *p, to->getAliveSiblings()) {
			if (p->getDefensiveHorse() && p->getDefensiveHorse()->objectName() == "god_horse" && p->getMark("Equips_Nullified_to_Yourself") == 0) {
				QString role = p->isLord() ? "loyalist" : p->getRole();
				if (from_role != role && to_role == role) {
					correct += 1;
				}
			}
		}
