...
public:
    ...
    bool triggerable(const ServerPlayer *target) const
    {
        return target != NULL && target->isAlive() && target->hasLordSkill(objectName()) && target->getMark("xingshuai_act") == 0 && hasWeiGens(target);
    }
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (player->askForSkillInvoke(objectName(), data)) {
            if (!player->isLord() && player->hasSkill("weidi")) {
				...
				//room->doLightbox("$XingshuaiAnimate");
            } else {
			...
			if (player->getMark(objectName()+"engine") > 0) {
				...
				foreach (ServerPlayer *wei, weis) {
					if (wei->askForSkillInvoke("_xingshuai", "xing")) {
						invokes << wei;
						room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, wei->objectName(), player->objectName());
		...
		if (player->getHp() > 0 && player->isLord() && !player->hasSkill("weidi")) {
...
{
    room->addPlayerMark(source, "huaiyiiengine");
    if (source->getMark("huaiyiiengine") > 0) {
        ...
        room->removePlayerMark(source, "huaiyiiengine");
    }
}
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (player->askForSkillInvoke(objectName())) {
            ...
            if (player->getMark(objectName()+"engine") > 0) {
                ...
                ServerPlayer *vic = room->askForPlayerChosen(player, mosts, objectName(), "@shifei-dis", true);
...
public:
    ...
    bool isEnabledAtPlay(const Player *player) const
    {
        ...
        return can_invoke &&  player->getMark("zhanjuedraw") < 2 && !player->isKongcheng();
    }
};
...
public:
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == PreDamageDone) {
            ...
            if (damage.from != NULL && damage.card != NULL && damage.card->isKindOf("Duel") && damage.card->getSkillName() == "zhanjue") {
...
public:
    ...
    const Card *viewAs(const Card *originalCard) const
    {
        ...
        qw->addSubcard(originalCard->getId());
        return qw;
    }
};
...
void YanzhuCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->addPlayerMark(effect.from, "yanzhuengine");
    if (effect.from->getMark("yanzhuengine") > 0) {
		if (!room->askForDiscard(target, "yanzhu", 1, 1, !target->getEquips().isEmpty(), true, "@yanzhu-discard")) {
			...
			if (effect.from->hasSkill("yanzhu", true)) {
				room->handleAcquireDetachSkills(effect.from, "-yanzhu");
			}
		}
        room->removePlayerMark(effect.from, "yanzhuengine");
    }
}
...
{
    room->addPlayerMark(source, "xingxueengine");
    if (source->getMark("xingxueengine") > 0) {
		if (Config.value("xingxue_down", true).toBool()) {
			foreach (ServerPlayer *t, targets) {
				if (t->isAlive() && !t->isNude()) {
					t->drawCards(1, "xingxue");
		...
		room->broadcastSkillInvoke(objectName());
		room->removePlayerMark(source, "xingxueengine");
    }
}
...
void YjYanyuCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    ...
    room->addPlayerMark(xiahou, "yjyanyuengine");
    if (xiahou->getMark("yjyanyuengine") > 0) {
		xiahou->drawCards(1, "recast");
		room->removePlayerMark(xiahou, "yjyanyuengine");
    }
...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        ...
        if (male != NULL) {
            room->broadcastSkillInvoke(objectName(), 2);
            room->addPlayerMark(player, "yjyanyuengine");
            if (player->getMark("yjyanyuengine") > 0) {
                male->drawCards(2, objectName());
                room->removePlayerMark(player, "yjyanyuengine");
            }
        }
...
void FurongCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->addPlayerMark(effect.from, "furongengine");
    if (effect.from->getMark("furongengine") > 0) {
		...
		delete c;
        room->removePlayerMark(effect.from, "furongengine");
    }
}
...
const Card *HuomoCard::validate(CardUseStruct &card_use) const
{
    ...
    room->addPlayerMark(zhongyao, "huomoengine");
    if (zhongyao->getMark("huomoengine") > 0) {
		...
		c->deleteLater();
        room->removePlayerMark(zhongyao, "huomoengine");
		return c;
    }
	return NULL;
}
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        foreach (ServerPlayer *zhongyao, room->getAllPlayers()) {
            if (TriggerSkill::triggerable(zhongyao) && player != zhongyao) {
                ...
                if (p != NULL) {
					room->broadcastSkillInvoke(objectName());
                    room->addPlayerMark(zhongyao, objectName()+"engine");
                    if (zhongyao->getMark(objectName()+"engine") > 0) {
                        p->drawCards(1, "zuoding");
                        room->removePlayerMark(zhongyao, objectName()+"engine");
                    }
				}
            }
        }
...
void AnguoCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->addPlayerMark(effect.from, "anguoengine");
    if (effect.from->getMark("anguoengine") > 0) {
		int beforen = 0;
		foreach (ServerPlayer *p, room->getAlivePlayers()) {
        ...
        room->removePlayerMark(effect.from, "anguoengine");
    }
}
