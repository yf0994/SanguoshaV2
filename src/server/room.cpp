...
void Room::killPlayer(ServerPlayer *victim, DamageStruct *reason) {
    ...
    if (Config.EnableHegemony) {
        ...
        log.arg = getTag(victim->objectName()).toString() == "Zhouyu_Buff" ? "junshi" : getTag(victim->objectName()).toString() == "Zhaoyun_Buff" ? "dajiang" : getTag(victim->objectName()).toString() == "Zhangyi_Buff" ? "zeishou" : victim->getRole();
    }
...
const Card *Room::askForSinglePeach(ServerPlayer *player, ServerPlayer *dying) {
    ...
    if (invoke) {
		foreach (ServerPlayer *p, getAlivePlayers()) {
			removePlayerCardLimitation(p, "use,response", "Peach$1");
		}
	}
    return result;
}
...
void Room::setPlayerProperty(ServerPlayer *player, const char *property_name, const QVariant &value) {
    ...
    if (strcmp(property_name, "maxhp") == 0 && !player->hasFlag("stop_trigger_maxhp"))
...
void Room::setPlayerMark(ServerPlayer *player, const QString &mark, int value) {
    ...
    bool invoke = mark == "BossMode_Boss" || thread->trigger(MarkChange, this, player, data); //fangzhi momingqimiao de shantui
	if (!invoke) {
...
void Room::assignGeneralsForPlayers(const QList<ServerPlayer *> &to_assign)
{
    ...
    if (Config.value("white_cloth", true).toBool()) {
        if (to_assign.first()->getGeneral()) {
            foreach (ServerPlayer *sp, m_players) {
                QStringList old_list = sp->getSelected();
                sp->clearSelected();
                ...
                //keep legal generals;
				QList<const Skill *> skilles;
				foreach (const Skill *skill, sp->getGeneral()->getVisibleSkillList()) {
                ...
                foreach (QString name, old_list) {
					...
					if ((Sanguosha->getGeneral(name)->getGender() == sp->getGeneral()->getGender() && Sanguosha->getGeneral(name)->getMaxHp() == sp->getGeneral()->getMaxHp() && skills.length() == skilles.length())) {
                        sp->addToSelected(name);
                        old_list.removeOne(name);
                    }
                }
    ...
    foreach (ServerPlayer *player, to_assign) {
		...
		int choice_count = qMin(max_choice + extra, max_available);
...
void Room::chooseGenerals(const QList<ServerPlayer *> &playerList) {
    //thread->trigger(Nothing, this, NULL);
...
void Room::run() {
    ...
    if (scenario && !scenario->generalSelection())
        ...
        startGame();
    } else if (mode == "06_boss") {
		QStringList gui_list;
		gui_list << "hundun" << "qiongqi" << "taowu" << "taotie";
		foreach (ServerPlayer *player, m_players) {
		...
		QStringList god_list;
		QStringList list;
		list << "shenguanyu" << "shenzhugeliang" << "shenlvbu" << "shenlvmeng" << "shencaocao" << "shenzhouyu" << "shensimayi" << "shenzhaoyun";
		for (int i = 0; i < Config.value("fuck_god_spinbox", 3).toInt(); ++i) {
			QString god = list.at(qrand()%list.length());
			list.removeOne(god);
			god_list << god;
		}
		foreach (ServerPlayer *player, m_players) {
			QStringList now_list;
			if (player->getRole() == "rebel") {
				now_list = Sanguosha->getRandomGenerals(5);
				foreach (const QString &name, god_list)
					now_list << name;
				QString general = askForGeneral(player, now_list);
				setPlayerProperty(player, "general", general);
...
void Room::assignRoles() {
    ...
    QStringList roles = Sanguosha->getRoleList(mode);
    if (mode != "08_defense" && mode != "06_boss") {
        qShuffle(roles);
    }
...
QString Room::_chooseDefaultGeneral(ServerPlayer *player) const{
    ...
    if (Config.EnableHegemony && Config.Enable2ndGeneral) {
        ...
        Q_ASSERT(false);
        return QString();
    } else if (Config.value("white_cloth", true).toBool()) {
        QStringList selectedNames = player->getSelected();
        foreach (const QString &name, selectedNames) {
            Q_ASSERT(!name.isEmpty());
            if (player->getGeneral() != NULL) { // choosing first general
				...
				foreach (const Skill *skill, Sanguosha->getGeneral(player->getGeneralName())->getVisibleSkillList()) {
					if (!skill->inherits("SPConvertSkill") && !skill->isAttachedLordSkill())
						skilles << skill;
				}
                if ((Sanguosha->getGeneral(name)->getGender() == Sanguosha->getGeneral(player->getGeneralName())->getGender() && Sanguosha->getGeneral(name)->getMaxHp() == Sanguosha->getGeneral(player->getGeneralName())->getMaxHp() && skills.length() == skilles.length()))
                    return name;
            } else {
                foreach (const QString &other, selectedNames) { // choosing second general
                    if (name == other) continue;
					...
					foreach (const Skill *skill, Sanguosha->getGeneral(other)->getVisibleSkillList()) {
						if (!skill->inherits("SPConvertSkill") && !skill->isAttachedLordSkill())
							skilles << skill;
					}
                    if (Sanguosha->getGeneral(name)->getGender() == Sanguosha->getGeneral(other)->getGender() && Sanguosha->getGeneral(name)->getMaxHp() == Sanguosha->getGeneral(other)->getMaxHp() && skills.length() == skilles.length())
                        return name;
                }
            }
        }
...
void Room::broadcastSkillInvoke(const QString &skill_name, bool isMale, int type) {
    ...
    if (player != NULL){
		if (player->tag["XiaodeSkill"].toString() == skill_name) {
			name = "xiaode";
			type = 2;
		}
		if (player->tag["HuashenSkill"].toString() == skill_name) {
			name = "huashen";
			type = 2;
		}
		if (player->hasSkill("bf_huashen")) {
			name = "bf_huashen";
			type = 2;
		}
	}
...
QMap<ServerPlayer *, const Card *> Room::multiPindianRace(ServerPlayer *from, QList<ServerPlayer *> tos, const QString &reason, bool include_from)
{
	QMap<ServerPlayer *, const Card *> result;
	QList<ServerPlayer *> all = tos;
	if (include_from)
		all.prepend(from);
    tryPause();
	...
	if (include_from) {
        ai = from->getAI();
		if (ai)
			result[from] = ai->askForPindian(from, reason);
		else
			all_ai = false;
	}
	...
	foreach (ServerPlayer *to, tos) {
		ai = to->getAI();
		if (ai)
			result[to] = ai->askForPindian(from, reason);
		else
			all_ai = false;
	}
    ...
    QList<ServerPlayer *> players;
    if (include_from && !result.contains(from)) {
        from->m_commandArgs = toJsonArray(from->objectName(), QString());
        players << from;
    }
	foreach (ServerPlayer *to, tos) {
