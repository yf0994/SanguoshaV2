    ...
    pindian_success(false), _m_currentStage(0)
{
    ...
    connect(ClientInstance, SIGNAL(skill_updated(const QString &skill_name)), this, SLOT(updateSkill(const QString &skill_name)));
...
void RoomScene::updateTargetsEnablity(const Card *card) {
    ...
    while (itor.hasNext()) {
        ...
        if (card) {
            if (card->isKindOf("Collateral") || card->isKindOf("rowanotherway"))
                isCollateral = true;
            else if (card->isKindOf("QiceCard")) {
                const QiceCard *qice_card = qobject_cast<const QiceCard *>(card);
                isCollateral = (qice_card->getUserString() == "collateral" || qice_card->getUserString() == "rowanotherway");
            }
        }
    ...
    if (skillName == "BossModeExpStore") {
       ...
       QHBoxLayout *layout2 = new QHBoxLayout;
        foreach (QString option, options) {
            ...
            QStringList validChoices = option.split("+");
		    foreach (QString choice, validChoices) {
			    QCommandLinkButton *button = new QCommandLinkButton;
                QString text = QString("%1:%2").arg(skillName).arg(choice);
                QString translated = Sanguosha->translate(text);
                if (text == translated)
                    translated = Sanguosha->translate(choice);
...
void RoomScene::keepGetCardLog(const CardsMoveStruct &move) {
    ...
    // private pile
    if (move.to_place == Player::PlaceSpecial && !move.to_pile_name.isNull() && !move.to_pile_name.startsWith('#') && !move.to_pile_name.startsWith('^^')) {
...
void RoomScene::useSelectedCard() {
    ...
    if (skill)
        ...
        foreach (const QString &pile,Self->getPileNames()){
            if (pile.startsWith("&") || pile.startsWith("^") || pile == "wooden_ox")
    ...
    if (victory) {
        foreach (const Player *player, ClientInstance->getPlayers()) {
            if (player->property("win").toBool()) {
				...
				else if (player->getGeneralName().contains("sunhao")) {
                    win_effect = "win-sunhao";
                }
				else if (player->getGeneralName().contains("ncaorui")) {
                    win_effect = "win-ncaorui";
                }
				else if (player->getGeneralName().contains("liuchen")) {
                    win_effect = "win-liuchen";
                }
				else if (player->getGeneralName().contains("sunxiu")) {
                    win_effect = "win-sunxiu";
                }
