...
void Dashboard::expandPileCards(const QString &pile_name) {
    ...
    foreach (CardItem *card_item, card_items) {
        ...
        card_item->setAcceptedMouseButtons(Qt::LeftButton | Qt::RightButton | Qt::MiddleButton);
    }
    bool prepend = true;
    if (new_name.startsWith("^") || new_name.startsWith("#")) {
        prepend = false;
    }
    foreach (CardItem *card_item, card_items)
...
void Dashboard::retractAllSkillPileCards()
{
    foreach (const QString &pileName, _m_pile_expanded) {
        if (!(pileName.startsWith("&") || pileName.startsWith("^") || pileName == "wooden_ox"))
...
void Dashboard::updatePending() {
    ...
    if (pending_card != new_pending_card) {
        ...
        }else if (view_as_skill->objectName().contains("jiaozhao")) {
			QString record_name = Self->property("jiaozhao_record_name").toString();
			if (record_name != "") {
				foreach (CardItem *item, m_handCards) {
                    item->hideFootnote();
                    if (new_pending_card && item->getCard() == cards.first()) {
                        item->setFootnote(Sanguosha->translate(record_name));
                        item->showFootnote();
                    }
                }
			}
