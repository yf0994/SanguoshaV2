...
void Settings::init() {
    ...
    OL = value("OL", false).toBool();
    MUSIC = value("MUSIC", false).toBool();
    ...
    EnableHidden = value("EnableHidden", true).toBool();
    EnableSuperConvert = value("EnableSuperConvert", true).toBool();
    ...
    QStringList roles_ban, kof_ban, hulao_ban, xmode_ban, god_ban, bossmode_ban, basara_ban, hegemony_ban, pairs_ban;
    ...
    god_ban = GetConfigFromLuaState(lua, "god_ban").toStringList();
    ...
    banlist = value("Banlist/06_boss").toStringList();
    if (banlist.isEmpty()) {
        foreach (const QString &ban_general, god_ban)
            banlist << ban_general;

        setValue("Banlist/06_boss", banlist);
    }
