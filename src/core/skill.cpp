...
QString Skill::getDescription() const
{
	...
	if (Config.value("heg_skill", true).toBool() && Sanguosha->translate(":" + QString("%1%2").arg(objectName()).arg("_heg")).contains(" "))
		name = QString("%1%2").arg(objectName()).arg("_heg");
	if (Config.value(objectName()+"_down", true).toBool() && Sanguosha->translate(":" + QString("%1%2").arg(objectName()).arg("_down")).contains(" "))
		name = QString("%1%2").arg(objectName()).arg("_down");
...
int TargetModSkill::getDistanceLimit(const Player *from, const Card *card, const Player *to) const{
  	return getDistanceLimit(from, card);
}
